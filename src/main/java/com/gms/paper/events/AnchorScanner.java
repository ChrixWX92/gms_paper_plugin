...
public class AnchorScanner implements Listener {
    ...
    public void onTeleport(PlayerTeleportEvent event) {
        ...
        if (event.getTo().getWorld().getName().equals(event.getFrom().getWorld().getName()) && !forceLoad) return;
        forceLoad = false;
        AnchorHandler.setLevelAnchors(new HashMap<>());
        if (event.getTo().getWorld() != null) {
            Log.info("ANCHORS: Scanning region files for Anchors...");
            File levelFolder = new File(Helper.getWorldPath(event.getTo().getWorld().getName()), "region");
